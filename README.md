...
**아쉬운 점**: `main`이 Vercel 자동배포와 바로 연결돼 있는데도, 대부분의 기간은 이슈/기능 단위가 아니라 담당자 1인당 브랜치 1개에 여러 변경을 누적하는 방식이었다. 진짜 기능 단위 분리(`capacitor-setup`, `android-back-button`)는 APK 패키징 단계에 와서야 처음 썼고, 병합된 브랜치도 지우지 않아 지금 6개가 원격에 그대로 남아있다. 브랜치 이름 규칙도 팀 전체가 지킨 게 아니라 사실상 한 명만 신경 쓴 셈이다 — 처음부터 기능 단위 브랜치 + `develop` 완충 단계 + 이름 규칙 통일을 했으면 더 좋았을 것 같다는 후회가 남는다.
