...
- 브랜치 이름 규칙: 카테고리 담당자는 자기 이름/카테고리 이니셜, 그 외 기능 작업은 `<기능명>` (예: `capacitor-setup`)
- 합치기 규칙: PR 생성 → 리뷰(주로 Claude Code가 diff 검토) → `main` 병합 — 병합 즉시 Vercel이 프로덕션에 자동 배포되기 때문에, 병합 전 검토가 실제로 사고를 막는 단계였다.

**실제로는 두 방식이 섞여 있었다는 점을 정직하게 밝힌다.** 프로젝트 대부분의 기간 동안은 "기능/이슈 단위 브랜치"가 아니라 **팀원 1인당 브랜치 1개**(`Medi`, `dongyoon`, `hibro`, `yeonsak`)를 계속 쓰면서, 그 사람이 담당한 여러 변경사항을 한 브랜치에 누적해서 올리고 주기적으로 `main`에 병합하는 방식이었다 — "기능 단위로 짧게 분리 → 리뷰 → 병합 → 삭제"로 버전을 관리하고 `main`의 상시 배포 가능 상태를 지킨다는 브랜치 전략의 취지에는 완전히 부합한다고 보기 어렵다. **한 브랜치 = 한 작업 단위**로 분리하는 방식은 프로젝트 막바지 APK 패키징 단계(`capacitor-setup`, `android-back-button`)에 와서야 처음 도입했고, 그 이후로는 검증이 끝난 작업 단위만 골라 `main`에 병합하는 흐름으로 진행했다.
- **개선할 점**: 처음부터 "담당자 1명 = 브랜치 1개"가 아니라 "이슈/기능 1개 = 브랜치 1개"로 더 잘게 쪼갰어야, `main`의 커밋 히스토리만 봐도 각 변경의 의도와 범위를 바로 추적할 수 있었을 것이다.

### 이슈 활용
총 12개 이슈, 전부 closed, 전부 라벨(`bug`/`feature`/`design`) 지정 완료:
...
### 다음 프로젝트에서 다르게 할 것
1. 브랜치를 담당자 1명당 1개가 아니라 이슈/기능 1개당 1개로 쪼갠다 — 지금은 프로젝트 막바지(APK 패키징) 가서야 이 방식을 처음 썼는데, 처음부터 이렇게 했어야 `main` 히스토리에서 각 변경의 범위가 바로 드러났을 것.
