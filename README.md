...
### 다음 프로젝트에서 다르게 할 것
1. 이슈에 라벨·담당자를 처음부터 GitHub 기능으로 지정한다(제목 태그로만 구분하지 않는다).
2. 새 바이너리 리소스 경로를 추가할 때는 커밋 전에 `.gitattributes` LFS 규칙부터 확인한다.
3. "전체 카테고리/전체 화면에 적용했다"는 커밋은 병합 전 카테고리별로 최소 1번씩 직접 확인한다.
