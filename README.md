...
| 8 | Android APK 패키징(Capacitor, 스플래시 애니메이션 포함) | 완료 |
...
### 이슈 활용
총 27개 이슈, 전부 closed. 라벨: `bug` 8 · `design` 12 · `feature` 4 · `question` 1 · 라벨 없음 2(#82, #84 — QA 막바지에 빠르게 등록하면서 라벨을 빼먹음):

| 이슈 | 라벨 | 상태 |
|---|---|---|
| #84 [스키마] 의료질문 카테고리에서 발생할수있는 오탐처리 | (없음) | 완료 |
| #82 [불일치] 화면비 구성의 기획과의 불일치 | (없음) | 완료 |
| #80 [버그] 프롬프트 예시 해시태그와 결과값의 불일치 | bug | 완료 |
| #74 [디자인] 스플래시 화면 디자인 요청(아이콘 팡 터지는 효과) | design | 완료 |
...
- 총 PR 수: **61개** — 58개 병합, 3개는 closed(미병합, #66·#67·#78 — 같은 변경을 올린 중복/재작업 PR이라 뒤에 올라온 PR로 대체되고 닫힘), open 0
...
| 모바일 AI 바로가기 버튼(챗GPT)이 앱과 브라우저를 동시에 열음 | 안드로이드 App Links 설정이 서비스마다 달라서(클로드만 앱으로 깔끔히 이동) 생기는 상대 서비스 쪽 동작 — `target="_blank"` 제거, `intent:` URI로 패키지 강제 지정 등 두 가지를 시도했지만 `intent:` URI는 WebView가 아예 이해 못 해 오히려 전부 무반응이 되는 역효과만 남 | **못 고침** — 두 시도 모두 결과에 영향이 없거나 악화시켜서 원복하고, 외부 서비스 제약으로 인정하고 넘어감(제미니·그록은 브라우저 단독으로는 열려서 실사용에 지장 없음) |
