// 의료질문 — 담당 우석민
// prd/의료질문.md 가 정본이다. 축 정의·키워드·겹침 처리 로직을
...
export const category: Category = {
  ...
  // 6개 예시가 축1(목적)×축2(대상)×축3(증상성격) 8가지 조합 중 서로 겹치지
  // 않는 6가지를 보여주도록 문장을 골랐다(2026-09-21 라이브러리 다양성
  // 점검) — 태그 자체는 바꾸지 않고 문장만 실제 키워드에 맞춰 조정.
  hashtags: [
    {
      tag: "#두통",
      example: "어제 저녁부터 갑자기 머리가 지끈거리는데 왜 그런지 모르겠어요.",
      // → 증상정보 / 본인 / 급성
    },
    {
      tag: "#소화",
      example: "몇 달째 소화가 안 되고 속이 더부룩한데 원인이 뭔지 궁금해요.",
      // → 증상정보 / 본인 / 만성
    },
    {
      tag: "#피부",
      example: "아이가 어제부터 갑자기 피부에 붉은 반점이 생겼는데 왜 그런지 모르겠어요.",
      // → 증상정보 / 가족 / 급성
    },
    {
      tag: "#수면",
      example:
        "몇 달째 잠을 잘 못 자다가 내일 병원 가는데 의사선생님께 뭐라고 설명해야 할지 모르겠어요.",
      // → 문진준비 / 본인 / 만성
    },
    {
      tag: "#운동",
      example:
        "아이가 오늘부터 갑자기 운동하다가 무릎을 다쳐서 병원 예약했는데, 진료 전에 정리하고 싶어서 여쭤봐요.",
      // → 문진준비 / 가족 / 급성
    },
    {
      tag: "#영양",
      example:
        "아이가 몇 달째 밥을 잘 안 먹어서 소아과 예약했는데, 진료 전에 정리하고 싶어서 어떻게 설명해야 할지 모르겠어요.",
      // → 문진준비 / 가족 / 만성
    },
  ],
}
...
// ("~때문인지"의 "~"는 선행 표현 자리표시일 뿐 리터럴 문자가 아니므로 제외)
// "인가요"는 삭제됨(오탐 감사, 2026-09-21) — "문진표인가요?"처럼 축2 우 신호
// 명사 뒤에도 흔히 붙는 계사 의문형이라, 겹침규칙(나중 신호 우선) 때문에
// 우 신호가 있는 문장까지 좌로 뒤집는 사례가 확인됨. 다른 좌 신호 구문들이
// 이미 의미를 충분히 커버해서 손실 없이 제거.
...
// 알려진 한계(팀 확인, 감수): "계속"은 증상의 만성 여부가 아니라 화자의 심리
// 상태를 수식할 때도 쓰여("어제부터 계속 신경쓰여서 심각한가요?" → 급성인데
// 만성으로 판정) 오탐 소지가 있으나, 자연어 모호함 영역이라 별도 로직 없이
// 감수하기로 함(prd/의료질문.md 축3 "알려진 한계" 참고).
