...
// "일정" 단독도 신호로 둔다 — "일정 짜줘"만 있으면 "일정도 좀 짜줘"처럼 조사·부사가
// 끼어드는 실제 문장(원문 그대로 substring 매칭이라 어절 삽입에 취약)을 놓친다.
const PURPOSE_RIGHT: Pattern[] = [
  "몇 박 며칠",
  "일정 짜줘",
  "일정",
  "동선",
  "시간표",
  /\d+박\s+\d+일/, // prd 정본 표기(공백 포함)를 그대로 따름 — 공백 없는 "5박6일"은 매치 안 함
]
...
const TIMING_RIGHT: Pattern[] = [
  ...
  "가볼까 하는데",
  "알아보는 중",
  /\d+(일|주|개월)\s+(후|뒤|남았는데|예정)/, // prd 정본 표기(공백 포함)를 그대로 따름
]

// 축4 — 여행 스타일(중복선택): 좌=휴양/힐링 / 우=액티비티/체험 중심
// "쉬고 싶어"를 "쉬고 싶"으로 줄여 어미 활용(싶어/싶은데/싶다 등)을 모두 잡는다.
const STYLE_LEFT: Pattern[] = ["쉬고 싶", "힐링", "여유롭게", "느긋하게", "아무것도 안 하고", "리조트에서"]
